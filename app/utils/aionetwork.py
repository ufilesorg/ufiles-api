...
async def aio_request_binary_stream(*, method: str = "get", url: str = None, **kwargs):
    async with aiohttp.ClientSession() as session:
        async for chunk in aio_request_binary_stream_session(
            session, method=method, url=url, **kwargs
        ):
            yield chunk


async def aio_request_binary_stream_session(
    session: aiohttp.ClientSession,
    *,
    method: str = "get",
    url: str = None,
